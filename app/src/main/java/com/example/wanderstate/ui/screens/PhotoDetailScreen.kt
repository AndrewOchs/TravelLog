...
import com.example.wanderstate.ui.viewmodel.PhotoDetailViewModel
import com.example.wanderstate.ui.viewmodel.ExportViewModel
import com.example.wanderstate.ui.viewmodel.ExportState
...
fun PhotoDetailScreen(
    ...
    val currentPhotoId by viewModel.currentPhotoId.collectAsState()
    ...
    LaunchedEffect(photoList, photoId) {
        if (photoList.isNotEmpty()) {
            ...
            val targetIndex = if (savedPageIndex >= 0 && savedPageIndex < photoList.size) {
                ...
                photoList.indexOfFirst { it.id == photoId }
            }
    ...
    LaunchedEffect(photoList, currentPhotoId) {
        if (photoList.isNotEmpty() && !photoList.any { it.id == currentPhotoId }) {
            Log.e("PhotoDetail", "CRITICAL: Current photo ($currentPhotoId) NOT FOUND in photo list! Navigating back. PhotoList IDs: ${photoList.map { it.id }}")
            onNavigateBack()
        }
    }
...
private fun PhotoInfoBottomSheet(
    photo: com.example.wanderstate.data.entities.PhotoEntity,
    journal: com.example.wanderstate.data.entities.JournalEntryEntity?,
    exportState: ExportState,
    contextType: String,
    onEditJournal: () -> Unit,
    onExportPhoto: () -> Unit,
